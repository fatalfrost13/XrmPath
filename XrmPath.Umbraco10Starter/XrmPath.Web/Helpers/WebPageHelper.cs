    ...
    public class WebPageHelper : BaseInitializer
    {
        ...
        public MetaDataModel GetMetaData(IPublishedContent content)
        {
            try
            {
                ...
                var title = pcUtil?.GetTitle(content);
                ...
                if (string.IsNullOrEmpty(title) || string.IsNullOrEmpty(description) || string.IsNullOrEmpty(keywords))
                {
                    ...
                    {
                        title = pcUtil?.GetTitle(homePage);
                    }
        ...
        public bool HandleUrlChange(IPublishedContent? content, string note = "")
        {
            ...
            if (autoGeneratedFolder != null)
            {
                ...
                else
                {
                    ...
                    var matchingNode = matchingNodes.OrderByDescending(i => i.ContentNode.CreateDate).FirstOrDefault(i => (bool)pcUtil?.NodeExists(i.ContentNode));
                    var removeNodes = matchingNodes.Where(i => i.ContentNode.Id != matchingNode.ContentNode.Id && (bool)pcUtil?.NodeExists(i.ContentNode));
