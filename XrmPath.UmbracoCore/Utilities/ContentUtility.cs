...
namespace XrmPath.UmbracoCore.Utilities
{
    ...
    public static class ContentUtility
    {
        ...
        {
            var contentTypes = ServiceUtility.ContentTypeService.GetAll();
            return contentTypes;
        }
        ...
        public static IEnumerable<IContent> GetChildren(this IContent content) {
            var children = ServiceUtility.ContentService.GetPagedChildren(content.Id, 1, 1000, out long totalRecords);
            return children;
        }
        ...
        {
            if (content == null || content.Id == 0 || content.GetChildren() == null) return null;
            return content.GetChildren().FirstOrDefault(child => child.NodeExists() && nodeTypeAliasSet.Contains(child.ContentType.Alias));
        }
        public static IContent FindChildNode(this IContent content, string nodeTypeAlias)
        {
            if (content == null || content.Id == 0 || content.GetChildren() == null) return null;
            return content.GetChildren().FirstOrDefault(child => child.NodeExists() && string.Equals(nodeTypeAlias, child.ContentType.Alias, StringComparison.Ordinal));
        }
        private static IEnumerable<IContent> FindAllNodesByAlias(this IContent content, ISet<string> nodeTypeAliasSet = null)
        {
            ...
            foreach (var child in content.GetChildren().SelectMany(child => FindAllNodesByAlias(child, nodeTypeAliasSet)))
                yield return child;
        }
        private static IEnumerable<IContent> FindAllNodesByAlias(this IContent content, string nodeTypeAlias = "")
        {
            ...
            foreach (var child in content.GetChildren().SelectMany(child => FindAllNodesByAlias(child, nodeTypeAlias)))
                yield return child;
        }
                ...
                if (!string.IsNullOrEmpty(contentValue))
                {
                    ...
                    else
                    {
                        ...
                        foreach (var udiValue in udisList)
                        {
                            ...
                            { 
                                var id = ServiceUtility.UmbracoHelper.Content(udi).Id;
        ...
        {
            if (!content.Published || !content.HasPendingChanges())
            {
                return true;
            }
            return false;
